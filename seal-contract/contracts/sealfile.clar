...
;; Batch 2: Private Helper Functions
...
;; Batch 5: Validation and Event Logging Helper Functions
...
;; Validate that the content hash is a 32-byte buffer
(define-private (validate-content-hash (hash (buff 32)))
    (is-eq (len hash) u32)
)

;; Validate that the signature hash is a 64-byte buffer
(define-private (validate-signature-hash (hash (buff 64)))
    (is-eq (len hash) u64)
)

;; Validate that metadata is non-empty
(define-private (validate-metadata (metadata (string-ascii 256)))
    (> (len metadata) u0)
)

;; Validate that the user is a valid principal and not the contract
(define-private (validate-user-principal (user principal))
    (and 
        (not (is-eq user tx-sender))  
        (not (is-eq user (as-contract tx-sender)))
...
;; Increment nonce for unique identifiers
(define-private (increment-nonce (nonce uint))
    (+ nonce u1)
)
